...
const Channel = dynamic(() => import("@/components/Channel/page"), {
  ssr: false,
});
const Comments = dynamic(() => import("@/components/Comments/page"), {
  ssr: false,
});
const Suggestion = dynamic(() => import("@/components/Suggestion/page"), {
  ssr: false,
});
const NotesEditor = dynamic(() => import("@/components/Notepad/page"), {
  ssr: false,
});
  ...
  useEffect(() => {
    async function addViews() {
      try {
        ...
        if (token) {
          await axios.get(
            `${process.env.NEXT_PUBLIC_BASE_URL}/videos/${id}`,
            {
              headers: {
          ...
          console.log("Video view updated and added to history");
        }
      } catch (error) {
