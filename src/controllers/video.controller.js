...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    if (!uploadedVideo)
        throw new ApiError(500, "Something went wrong while saving video data");
    return res
        .status(200)
        .json(
            new ApiResponse(
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    if (!video)
        throw new ApiError(
            500,
            "Something went wrong while updating video data"
        );
    return res
        .status(200)
        .json(
...
const deleteVideo = asyncHandler(async (req, res) => {
    ...
    const deletedVideoUrl = deletedVideo.videoFile;
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                video,
                "Pusblished status changed Successfully"
            )
        );
});
